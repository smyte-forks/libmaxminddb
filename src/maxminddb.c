...
LOCAL int read_file(MMDB_s *const mmdb);
...
int MMDB_open(const char *const filename, uint32_t flags, MMDB_s *const mmdb)
{
    ...
    if ((flags & MMDB_MODE_MASK) == MMDB_MODE_MEMORY) {
        status = read_file(mmdb);
    } else {
        status = map_file(mmdb);
    }
...
LOCAL int read_file(MMDB_s *const mmdb)
{
    ...
    file_content = (uint8_t *) malloc(size);
    if (file_content == NULL) {
        status = MMDB_OUT_OF_MEMORY_ERROR;
        goto cleanup;
    }
    if (fread(file_content, size, 1, f) < 1) {
        status = MMDB_IO_ERROR;
        goto cleanup;
    }
...
LOCAL void free_mmdb_struct(MMDB_s *const mmdb)
{
    ...
    if (NULL != mmdb->file_content) {
        ...
        WSACleanup();
#else
        if ((mmdb->flags & MMDB_MODE_MASK) == MMDB_MODE_MEMORY) {
